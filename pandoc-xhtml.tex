...
\startxmlsetups xml:body
    % Inside <body>-tags we want to process all elements we know.
    \xmlflush{#1}

    % Afterwards (i.e when the tag closes), we should cleanup dangling
    % section blocks. If a section block is not closed, the last page
    % might not be set correctly.
    \cleanupsectionblocks
\stopxmlsetups
...
\startxmlsetups xml:margin
     \margintext{\xmlflush{#1}}
\stopxmlsetups
...
\startxmlsetups xml:img:float
    \startplacefigure[reference=\xmlattribute{#1}{/img}{id}, title=\xmltext{#1}{/p[@class='caption']}]
        \xmlflush{#1}
    \stopplacefigure
\stopxmlsetups
...
%%% Cleanup dangling section blocks
\starttexdefinition cleanupsectionblocks
  \doifmode{*whatcomesfirst}{\stopfirstmatter}
  \doifmode{*frontpart}{\stopfrontmatter}
  \doifmode{*bodypart}{\stopbodymatter}
  \doifmode{*backpart}{\stopbackmatter}
\stoptexdefinition

%\appendtoks\cleanupsectionblocks\to\everylastshipout
